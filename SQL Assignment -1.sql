...
use mavenmovies; 
 ...
 -- primary key is a unique identifier generally is focuse on the uniqueness of the table its assures 
--  that the value in a specific column is unique -- a table allows only one primary key in a table 
 -- FOKREIGN KEY is used to build relationship bewteen two tables -- Tables can allows more then one foreign key  
...
desc film_category; -- film_id and category_id is primary key 
...
select  count(film_id) as Total_no_of_film from film where replacement_cost>15 and 
replacement_cost<20; 
